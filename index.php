<?php
  ...
  $stmt = $pdo->prepare("
    SELECT 
      tracks.*,
      GROUP_CONCAT(DISTINCT all_artists.stage_name ORDER BY all_artists.stage_name SEPARATOR ', ') AS all_artists
    FROM tracks
    ...
    LEFT JOIN artists AS featured_artist ON track_features.artist_id = featured_artist.id
    LEFT JOIN (SELECT id, stage_name FROM artists) AS all_artists 
    ON all_artists.id = tracks.artist_id OR all_artists.id = track_features.artist_id
